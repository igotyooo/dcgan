...
Z = T.matrix(  )
IT_zr = T.tensor4(  )
IT_zf = decoder( Z, *decoder_params )
YD_zr = discrim( IT_zr, *discrim_params )
YD_zf = discrim( IT_zf, *discrim_params )
IST_zr = T.concatenate( [ IS, IT_zr ], axis = 1 )
IST_zf = T.concatenate( [ IS, IT_zf ], axis = 1 )
YDD_zr = domain_discrim( IST_zr, *domain_discrim_params )
YDD_zf = domain_discrim( IST_zf, *domain_discrim_params )
# For converter-decoder.
cost_d_for_cd_zf = bce( YD_zf, T.ones( YD_zf.shape ) ).mean(  )
cost_dd_for_cd_zf = bce( YDD_zf, T.zeros( YDD_zf.shape ) ).mean(  )
cost_for_cd_z = cost_d_for_cd_zf + cost_dd_for_cd_zf
cd_z_updater = updates.Adam( lr = lrt, b1 = b1, regularizer = updates.Regularizer( l2 = l2 ) )
cd_z_updates = cd_z_updater( decoder_params, cost_for_cd_z )
# For discriminator.
cost_d_for_d_zr = bce( YD_zr, T.ones( YD_zr.shape ) ).mean(  )
cost_d_for_d_zf = bce( YD_zf, T.zeros( YD_zf.shape ) ).mean(  )
cost_for_d_z = cost_d_for_d_zr + cost_d_for_d_zf
d_z_updater = updates.Adam( lr = lrt, b1 = b1, regularizer = updates.Regularizer( l2 = l2 ) )
d_z_updates = d_z_updater( discrim_params, cost_for_d_z )
# For domain-discriminator.
cost_dd_for_dd_zr = bce( YDD_zr, T.zeros( YDD_zr.shape ) ).mean(  )
cost_dd_for_dd_zf = bce( YDD_zf, T.zeros( YDD_zf.shape ) ).mean(  )
cost_for_dd_z = cost_dd_for_dd_zr + cost_dd_for_dd_zf
dd_z_updater = updates.Adam( lr = lrt, b1 = b1, regularizer = updates.Regularizer( l2 = l2 ) )
dd_z_updates = dd_z_updater( domain_discrim_params, cost_for_dd_z )

# DEFINE FORWARD (COSTS) AND BACKWARD (UPDATE) FROM SOURCE IMAGE.
IT_sr = T.tensor4(  )
...
print 'COMPILING'
t = time(  )
_train_cd_z = theano.function( [ Z, IS ], cost_for_cd_z, updates = cd_z_updates )
_train_d_z = theano.function( [ Z, IT_zr ], cost_for_d_z, updates = d_z_updates )
_train_dd_z = theano.function( [ Z, IS, IT_zr ], cost_for_dd_z, updates = dd_z_updates )
...
_test_cd = theano.function( [ Z ], IT_zf )
...
di_st = Pldt(  )
di_st.set_LOOKBOOK(  )
ims_st = di_st.load( npx, True )
print( 'Swap dimension of %s.' % di_st.name )
ims_st = tf( ims_st, npx )
print( 'Done.' )
di_st.shuffle(  )
...
di_zt.set_PRODUCTS(  )
di_zt.shuffle(  )
ims_zt = di_zt.load( npx, True )
print( 'Swap dimension of %s.' % di_zt.name )
ims_zt = tf( ims_zt, npx )
print( 'Done.' )
...
log_fields = [ '00__num_epoch', '01__num_update', '02__num_example', '03__t_spent', 
            '04__tr_cost_fs', '05__tr_cost_d_fs', '06__tr_cost_dd_fs', 
            '07__tr_cost_fz', '08__tr_cost_d_fz', '09__tr_cost_dd_fz',
            '10__val_cost_fs', '11__val_cost_d_fs', '12__val_cost_dd_fs' ]
...
vis_zb = floatX( np_rng.uniform( -1., 1., size = ( nvis ** 2, nz ) ) )
vis_ims_z_hat = _test_cd( vis_zb )
color_grid_vis( itf( vis_ims_z_hat, npx ), ( nvis, nvis ),
            os.path.join( sample_dir, 'Z000T.png' ) )

# DO THE JOB.
print desc.upper(  )
num_epoch = 0
num_sample_zt = len( ims_zt )
t = time(  )
for epoch in range( niter ):
    ...
    if os.path.exists( mpath_ce ) and os.path.exists( mpath_cd ) and os.path.exists( mpath_d ) and os.path.exists( mpath_dd ):
    ...
    cost_for_cd_z_cumm = 0.
    cost_for_d_z_cumm = 0.
    cost_for_dd_z_cumm = 0.
    ...
    for bi in range( num_batch ):
        ...
        cost_for_cd_z_cumm += cost_for_cd_z * this_bsize
        cost_for_ced_s_cumm += cost_for_ced_s * this_bsize
        # Train discriminator.
        cost_for_d_z = _train_d_z( Zb, ITb_zr ) # Update d * 2
        cost_for_d_s = _train_d_s( ISb, ITb_sr ) # Update d * 2
        cost_for_d_z_cumm += cost_for_d_z * this_bsize
        cost_for_d_s_cumm += cost_for_d_s * this_bsize
        # Train domain-discriminator.
        cost_for_dd_z = _train_dd_z( Zb, ISb, ITb_zr ) # Update dd * 2
        cost_for_dd_s = _train_dd_s( ISb, ITb_sr ) # Update dd * 2
        cost_for_dd_z_cumm += cost_for_dd_z * this_bsize
        cost_for_dd_s_cumm += cost_for_dd_s * this_bsize
        # Monitor.
        if np.mod( bi, num_batch / 20 ) == 0:
            prog = np.round( bi * 100. / num_batch )
            print( 'Epoch %d: cd_z = %.4f, ced_s = %.4f, d_z = %.4f, d_s = %.4f, dd_z = %.4f, dd_s = %.4f  %d%% (%d/%d)' 
                    % ( num_epoch, cost_for_cd_z, cost_for_ced_s, cost_for_d_z, cost_for_d_s, cost_for_dd_z, cost_for_dd_s, prog, bi + 1, num_batch ) )
    # Save network.
    cost_for_cd_z_cumm /= num_sample_st
    cost_for_ced_s_cumm /= num_sample_st
    cost_for_d_z_cumm /= num_sample_st
    cost_for_d_s_cumm /= num_sample_st
    cost_for_dd_z_cumm /= num_sample_st
    cost_for_dd_s_cumm /= num_sample_st
    print( 'Epoch %d: cd_z = %.4f, ced_s = %.4f, d_z = %.4f, d_s = %.4f, dd_z = %.4f, dd_s = %.4f' 
            % ( num_epoch, cost_for_cd_z_cumm, cost_for_ced_s_cumm, cost_for_d_z_cumm, cost_for_d_s_cumm, cost_for_dd_z_cumm, cost_for_dd_s_cumm ) )
    ...
    cost_for_d_s_cumm /= num_sample_st
    cost_for_dd_s_cumm /= num_sample_st
    print( 'Val) Epoch %d: cd_z = N/A, ced_s = %.4f, d_z = N/A, d_s = %.4f, dd_z = N/A, dd_s = %.4f' 
            % ( num_epoch, cost_for_ced_s_cumm, cost_for_d_s_cumm, cost_for_dd_s_cumm ) )
    # Sample visualization.
    samples = np.asarray( _test_ced( vis_ims_tr_s ) )
    color_grid_vis( itf( samples, npx ), ( nvis, nvis ),
            os.path.join( sample_dir, 'TR%03dT.png' % num_epoch ) )
    samples = np.asarray( _test_ced( vis_ims_val_s ) )
    color_grid_vis( itf( samples, npx ), ( nvis, nvis ),
            os.path.join( sample_dir, 'VAL%03dT.png' % num_epoch ) )
    samples = np.asarray( _test_cd( vis_zb ) )
    color_grid_vis( itf( samples, npx ), ( nvis, nvis ),
            os.path.join( sample_dir, 'Z%03dT.png' % num_epoch ) )
