...
cg5 = gain_ifn( nz, 'cg5' )
cb5 = bias_ifn( nz, 'cb5' )
encoder_params = [ cw0, cw1, cg1, cb1, cw2, cg2, cb2, cw3, cg3, cb3, cw4, cg4, cb4, cw5, cg5, cb5 ]
def converter( IS, w0, w1, g1, b1, w2, g2, b2, w3, g3, b3, w4, g4, b4, w5, g5, b5 ):
    ...
    h3d = relu( batchnorm( deconv( h4d, cw3d, subsample = ( 2, 2 ), border_mode = ( 2, 2 ) ), g = cg3d, b = cb3d ) )
    h2d = relu( batchnorm( deconv( h3d, cw2d, subsample = ( 2, 2 ), border_mode = ( 2, 2 ) ), g = cg2d, b = cb2d ) )
    h1d = tanh( deconv( h2d, cw1d, subsample = ( 2, 2 ), border_mode = ( 2, 2 ) ) )
    y = h1d
    return y
...
print 'COMPILING'
t = time(  )
_train_e = theano.function( [ IS, IT ], cost, updates = e_updates )
...
for epoch in range( niter ):
    ...
    if os.path.exists( mpath ):
        ...
        for pi in range( len( encoder_params ) ):
            encoder_params[ pi ].set_value( data[ pi ] )
        continue
    # Training.
    ...
    # Leave logs.
    e_cost = float( cost )
    log = [ num_epoch, num_update, num_example, time(  ) - t, e_cost ]
    ...
    np.save( mpath, [ p.get_value(  ) for p in encoder_params ] )
