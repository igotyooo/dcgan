...
for epoch in range( niter ):
    ...
    for bi in range( num_batch ):
        ...
        # Train converter.
        cost_for_ced_s = _train_ced_s( ISb, ITb_sr ) # Update ced * 2
        ...
        if np.mod( bi, num_batch / 20 ) == 0:
            ...
            print( 'Epoch %d: ced_s = %.4f, d_s = %.4f, dd_s = %.4f  %d%% (%d/%d)' 
                    % ( num_epoch, cost_for_ced_s, cost_for_d_s, cost_for_dd_s, prog, bi + 1, num_batch ) )
    # Save network.
    cost_for_ced_s_cumm /= num_sample_st
    cost_for_d_s_cumm /= num_sample_st
    cost_for_dd_s_cumm /= num_sample_st
    print( 'Epoch %d: ced_s = %.4f, d_s = %.4f, dd_s = %.4f' 
    ...
    for bi in range( num_batch ):
        ...
        for b in range( this_bsize ):
            ...
            ITb_sru[ b ] = ims_st[ iid ]
        # Val converter.
        cost_for_ced_s = _val_ced_s( ISb, ITb_sr )
        cost_for_ced_s_cumm += cost_for_ced_s * this_bsize
        # Val discriminator.
    ...
    cost_for_d_s_cumm /= num_sample_st
    cost_for_dd_s_cumm /= num_sample_st
    print( 'Val) Epoch %d: ced_s = %.4f, d_s = %.4f, dd_s = %.4f' 
            % ( num_epoch, cost_for_ced_s_cumm, cost_for_d_s_cumm, cost_for_dd_s_cumm ) )
